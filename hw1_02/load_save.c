...
int open_file(FILE **f, open_t open)
{
    ...
    if (!(home = getenv("HOME")))
    {
        printf("Home Enviroment Variable is null.\n");
    ...
    case read:
        if (!(*f = fopen(path, "r")))
        {
            printf("Dungeon not found in default location.\n");
            open_file_working_directory(&f, open);
            return 1;
        }
        break;
...
int open_file_working_directory(FILE ***f, open_t open)
{
    printf("Defaulting to the working directory.\n");
    switch (open)
    {
    case read:
