...
int load_dungeon(Dungeon *d)
{
    ...
    fread(&d->player.x, sizeof(uint8_t), 1, f);    //Player x
    fread(&d->player.y, sizeof(uint8_t), 1, f);    //Player y
    ...
    d->rooms = (Room *)malloc(sizeof(Room) * d->num_rooms);
    ...
    d->upstairs = (Position *)malloc(sizeof(Position) * d->num_upstairs);
    ...
    d->downstairs = (Position *)malloc(sizeof(Position) * d->num_downstairs);
...
void open_file(FILE **f, open_t open)
{
    ...
    if (!(path = (char *)malloc(strlen(home) + strlen("/.rlg327/dungeon") + 1)))
    {
