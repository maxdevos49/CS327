...
void gaussian_filter(Dungeon *d, void *h);
...
void place_rooms(Dungeon *d)
{
    int attempts = 0;

    d->rooms = (Room *)malloc(sizeof(Room) * MAX_ROOMS);
...
void add_corridor(Dungeon *d, uint8_t x, uint8_t y)
{
...
void place_stairs(Dungeon *d)
{
    ...
    d->num_upstairs = 1;
    d->upstairs = (Position *)malloc(sizeof(Position));
    d->upstairs[0].x = rx;
    d->upstairs[0].y = ry;
...
{
    d->characters[y][x] = (Character *)malloc(sizeof(Character));
