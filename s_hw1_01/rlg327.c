...
#include <sys/stat.h>
#include <sys/types.h>
#include <limits.h> //defines MAX_INT
#include <sys/time.h>
#include <assert.h>
...
static int32_t corridor_path_cmp(const void *key, const void *with) //compare function
{
    return ((corridor_path_t *)key)->cost - ((corridor_path_t *)with)->cost;
}

static void dijkstra_corridor(dungeon_t *d, pair_t from, pair_t to) //implementation
{
    ...
    if (!initialized)
    {
        ...
        {
            for (x = 0; x < DUNGEON_X; x++) //init corridor_path_t array with corrdinate locations
            {
                path[y][x].pos[dim_y] = y;
                path[y][x].pos[dim_x] = x;
            }
        }
        initialized = 1; //let it know its been inited
    }
    ...
    for (y = 0; y < DUNGEON_Y; y++)
    {
        for (x = 0; x < DUNGEON_X; x++)
        {
            ...
            path[y][x].cost = INT_MAX; //INT_MAX defined in #include <limits.h>
        }
    }
    //set starting location to cost 0, from is type pair_t
    ...
    for (y = 0; y < DUNGEON_Y; y++)
    {
        for (x = 0; x < DUNGEON_X; x++)
        {
            if (mapxy(x, y) != ter_wall_immutable) //fill heap if not hard wall
            {
    ...
    while ((p = heap_remove_min(&h)))
    {
        ...
        if ((p->pos[dim_y] == to[dim_y]) && p->pos[dim_x] == to[dim_x])//if target is reached
        {
            for (x = to[dim_x], y = to[dim_y]; (x != from[dim_x]) || (y != from[dim_y]); p = &path[y][x], x = p->from[dim_x], y = p->from[dim_y])
            {
                if (mapxy(x, y) != ter_floor_room)
                {
                    mapxy(x, y) = ter_floor_hall;//add corridor
                    hardnessxy(x, y) = 0;
                }
            }
            heap_delete(&h);//done at this point
            return;
        }
