...
int move_player(turn_t *p, dungeon_t *d)
{
    ...
    case 60: //up
        ...
        {
            d->mode = m_gen_floor;
            d->hud.status = "You traveled down a floor";
            return 0;
        }
        d->hud.status = "You can not go up there.";
        update_hud(d);
        return 1;
        break;
    case 62: //down
        ...
        {
            d->mode = m_gen_floor;
            d->hud.status = "You traveled up a floor";
            return 0;
        }
        d->hud.status = "You can not go down there.";
        update_hud(d);
        return 1;
        break;
    ...
    {
        d->hud.status = "Rock is in the way!";
        update_hud(d);
    }
    else
    {
